...
import com.example.myapplication.Chat.Dto.MessageDto;
import com.example.myapplication.Chat.Dto.RoomDto;
import com.example.myapplication.Chat.retrofit.Result;
...
public class ChatActivity extends AppCompatActivity {
    ...
    private void init() {
        ...
        mSocket.on(Socket.EVENT_CONNECT, args -> {
            mSocket.emit("enter", gson.toJson(new RoomDto(username, roomNumber)));
        });
        mSocket.on("update", args -> {
            MessageDto data = gson.fromJson(args[0].toString(), MessageDto.class);
            addChat(data);
        });
    }

    // 리사이클러뷰에 채팅 추가
    private void addChat(MessageDto data) {
        runOnUiThread(() -> {
    ...
    private void sendMessage() {
        mSocket.emit("newMessage", gson.toJson(new MessageDto("MESSAGE",
                username,
                roomNumber,
                binding.contentEdit.getText().toString(),
                System.currentTimeMillis())));
        Log.d("MESSAGE", new MessageDto("MESSAGE",
                username,
                roomNumber,
                binding.contentEdit.getText().toString(),
    ...
    private void sendImage(String imageUri) {
        mSocket.emit("newImage", gson.toJson(new MessageDto("IMAGE",
                username,
                roomNumber,
                imageUri,
                System.currentTimeMillis())));
        Log.d("IMAGE", new MessageDto("IMAGE",
                username,
                roomNumber,
                imageUri,
                System.currentTimeMillis()).toString());
    ...
    protected void onDestroy() {
        super.onDestroy();
        mSocket.emit("left", gson.toJson(new RoomDto(username, roomNumber)));
        mSocket.disconnect();
    }
}
